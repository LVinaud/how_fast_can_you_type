	...
	Storei Rx, Ry
			...
			free(str_tmp2);
                ...
                */
                ...
                /* RAND Rx : le 1 registrador, igual ao INCHAR */
                case RAND_CODE :
                    str_tmp1 = parser_GetItem_s();
                    val1 = BuscaRegistrador(str_tmp1);
                    free(str_tmp1);
                    str_tmp1 = ConverteRegistrador(val1);
                    sprintf(str_msg,"%s%s0000000",RAND,str_tmp1);
                    free(str_tmp1);
                    parser_Write_Inst(str_msg,end_cnt);
                    end_cnt += 1;
                    break;

                /* RDTIME Rx : le 1 registrador, igual ao INCHAR */
                case RDTIME_CODE :
                    str_tmp1 = parser_GetItem_s();
                    val1 = BuscaRegistrador(str_tmp1);
                    free(str_tmp1);
                    str_tmp1 = ConverteRegistrador(val1);
                    sprintf(str_msg,"%s%s0000000",RDTIME,str_tmp1);
                    free(str_tmp1);
                    parser_Write_Inst(str_msg,end_cnt);
                    end_cnt += 1;
                    break;
...
int BuscaInstrucao(char * nome)
{
    ...
    */
    ...
    else if (strcmp(str_tmp,RDTIME_STR) == 0)
    {
        return RDTIME_CODE;
    }
